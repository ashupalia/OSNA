...
def bfs(graph, root, max_depth):
    ...
    usee = []
    visited = set()
    number_of_paths[root]=1
    
    while len(q) > 0 and max_depth>=depth:
            current_node=q.pop()
            if current_node not in seen:
                seen.append(current_node) 
                usee.append(current_node)
                for a in g.neighbors(current_node):  
                    if a not in seen:
                        q.appendleft(a) 
                        
                        if current_node not in distance_root:
                            a_dict[a]=current_node+"(,)"+a
                            dep = len(a_dict[a].split('(,)'))
                            depth= dep - 1
                            flag == 1
                            if max_depth>=depth:  
                                number_of_paths[a]=1  
                                parent_node[a].append(current_node)                                                                         
                                distance_root[a]=1
                            
                        else:     
                            if a in distance_root:
                                a1 = len(a_dict[current_node].split('(,)'))+1
                                b1 = len(a_dict[a].split('(,)'))
                                if a1==b1:
                                    parent_node[a].append(current_node)         
                                    number_of_paths[a]=number_of_paths[a]+1
                                
                            else:
                                a_dict[a]=a_dict[current_node]+"(,)"+a
                                dep = len(a_dict[a].split('(,)'))
                                depth= dep-1
                                if depth<=max_depth:
                                    parent_node[a].append(current_node)
                                    number_of_paths[a]=1
                                    distance_root[a]=distance_root[current_node]+1


    return distance_root,number_of_paths,parent_node
    pass
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for a in node2distances:

        nodec[a] = 1

        if node2distances[a] not in a_dict:
            nam.append(a)
            a_dict[node2distances[a]] = nam
            
        else:
            nam = a_dict[node2distances[a]]
            nam.append(a)
            a_dict[node2distances[a]] = nam
    ...
    for i in sorted_dict:
        if (i!= 0):
            ...
            q = [1,2,3]
            for a in equal:
                father = node2parents[a]
                for parent in father:
                    t = a, parent
                    t = sorted(t)
                    output[(t[0], t[1])] = float(nodec[a] / len(father))
                    nodec[parent] = float(nodec[parent]) + float(nodec[a] / len(father))
...
def partition_girvan_newman(graph, max_depth):
    ...
    first_dic={}
    g=graph.copy()
    first_dic=approximate_betweenness(g, max_depth)
    asd = dict()
    qrm = list() 
    ...
    while nx.number_connected_components(g)<=1:           
        ...
        for a in first_dic:         
            if g.has_edge(a[0],a[1]) or g.has_edge(a[1],a[0]):
                if float(first_dic[a])>=float(maximum):
                    if float(first_dic[a])!=float(maximum):
                        maxdepth['val']=a
                        maximum=float(first_dic[a])
                        
                   
                    else:
                        if a[0]<maxdepth['val'][0]:
                            maxdepth['val']=a
                            maximum=float(first_dic[a])
                         
                        if a[0]==maxdepth['val'][0]:
                            if a[1]<maxdepth['val'][1]:
                                maxdepth['val']=a
                                maximum=float(first_dic[a])
...
def get_subgraph(graph, min_degree):
    ...
    min_val= 0
    g=graph.copy()
    number =  [node for node in g if g.degree(node)<min_degree]
    g.remove_nodes_from(number)
    return g 
...
def volume(nodes, graph):
    ...
    volume = 0
    for node in nodes:
        volume = volume + len(g.edges(node))
        g.remove_node(node)
    return volume
    pass
...
def cut(S, T, graph):
    ...
    first_graph = graph.subgraph(S)
    first_edges = first_graph.number_of_edges()
    
    second_graph = graph.subgraph(T)
    second_edges = second_graph.number_of_edges()
    
    return number_of_edges - first_edges - second_edges
    pass
...
def score_max_depths(graph, max_depths):
    ...
    for a in max_depths:
        ...
        components=partition_girvan_newman(graph, a)
        cutdeep=()
        cutdeep=(a, norm_cut(components[0].nodes(),components[1].nodes(), graph))
        newshortlist.append(cutdeep)
...
def make_training_graph(graph, test_node, n):
    ...
    j = 0
    sort_nodes = sorted(train_graph.neighbors(test_node))
    
    parent = list()
    for neighbors in sort_nodes:
        train_graph.remove_edge(neighbors, test_node)
        count = count + 1
        if count == n:
            break
    #[train_graph.remove_edge(neighbors, test_node) for neighbors in sorted(train_graph.neighbors(test_node)) if i < n ]
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        ...
        if n != node and n not in neighborsi :
    ...
    count=0  
    ra = len(scores)
    while count<k and count<ra:
        final_score.append((scores[count]))
        count=count+1
...
def path_score(graph, root, k, beta):
    ...
    for nodes in g.nodes():
        if nodes in node2distances and nodes in node2num_paths:
            calculation=math.pow(beta,node2distances[nodes])
            value_of_sum= float(calculation*node2num_paths[nodes])
            total.append(((root,nodes),value_of_sum))
        
        
            
    total= sorted(sorted(total,key=lambda  x: x[0]),key=lambda x:x[1] , reverse=True)
...
def evaluate(predicted_edges, graph):
    ...
    """
    ###TODO
    a =  len([x for x in predicted_edges if graph.has_edge(x[0], x[1]) or graph.has_edge(x[1], x[0])]) 
...
def main():
    ...
    print('first partition: cluster 1 has %d nodes and cluster 2 has %d nodes' %
          (clusters[1].order(), clusters[0].order()))
